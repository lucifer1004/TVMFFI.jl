...
function print_comparison(julia_results::Dict{String, Float64})
    ...
    comparisons = [
        ...
        ("TVM GPU autodlpack (CuArray)", "gpu_autodlpack",
            "tvm_ffi.nop.autodlpack(torch[cuda])"),
    ...
    for (name, julia_key, python_key) in comparisons
        ...
        if !isnan(julia_val) && !isnan(python_val)
            ...
            speedup_str = speedup >= 1.0 ? @sprintf("%.1fx faster", speedup) :
                          @sprintf("%.1fx slower", 1/speedup)
            # Use appropriate unit
            julia_unit = julia_val >= 1e-6 ? (@sprintf("%.1f µs", julia_val*1e6)) :
                         (@sprintf("%.0f ns", julia_val*1e9))
            python_unit = python_val >= 1e-6 ? (@sprintf("%.1f µs", python_val*1e6)) :
                          (@sprintf("%.0f ns", python_val*1e9))
...
function bench_and_record!(
        results::Dict{String, Float64}, key::String, name::String, f::Function, repeat::Int)
...
function main()
    ...
    nop = get_global_func("testing.nop")
    bench_and_record!(
        results, "nop_no_args", "tvm_ffi.nop() [no args]", () -> nop(), repeat)
