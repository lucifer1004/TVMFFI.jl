...
@testset "Test Fixtures" begin
    @testset "add_one_cpu - Basic" begin
        ...
        x = Float32[1, 2, 3, 4, 5]
        y = similar(x)
        add_one(TensorView(x), TensorView(y))
        @test y ≈ Float32[2, 3, 4, 5, 6]

        # Test: [10, 20, 30] + 1 = [11, 21, 31]
        x2 = Float32[10, 20, 30]
        y2 = similar(x2)
        add_one(TensorView(x2), TensorView(y2))
        @test y2 ≈ Float32[11, 21, 31]

        # Test: zeros + 1 = ones
        x3 = zeros(Float32, 10)
        y3 = similar(x3)
        add_one(TensorView(x3), TensorView(y3))
    ...
    @testset "add_one_cpu - Strided Arrays" begin
        ...
        add_one(TensorView(x_slice), TensorView(y_slice))
        ...
        add_one(TensorView(x_strided), TensorView(y_strided))
        @test y_strided ≈ Float32[2, 4, 6, 8]

        # Test 3: 2D contiguous array
        x_mat = Float32[1 2 3; 4 5 6]  # 2×3
        y_mat = similar(x_mat)
        add_one(TensorView(x_mat), TensorView(y_mat))
        ...
        y_sub = similar(x_sub)
        add_one(TensorView(x_sub), TensorView(y_sub))
        @test y_sub ≈ Float32[7 8; 11 12]

        # Test 7: 3D array
        x_3d = reshape(Float32[1:24...], 2, 3, 4)  # 2×3×4
        y_3d = similar(x_3d)
        add_one(TensorView(x_3d), TensorView(y_3d))
        @test y_3d ≈ x_3d .+ 1
        ...
        add_one(TensorView(x_3d_slice), TensorView(y_3d_slice))
        ...
        add_one(TensorView(x_complex), TensorView(y_complex))
    ...
    @testset "add_one_cuda - GPU Arrays (Optional)" begin
        if !CUDA_AVAILABLE
            ...
            add_one_cuda(TensorView(x_gpu), TensorView(y_gpu))
            ...
            add_one_cuda(TensorView(x_strided), TensorView(y_strided))
            ...
            add_one_cuda(TensorView(x_mat_gpu), TensorView(y_mat_gpu))
            ...
            add_one_cuda(TensorView(x_col), TensorView(y_col))
    ...
    @testset "add_one_metal - GPU Arrays (Optional)" begin
        if !METAL_AVAILABLE
            ...
            add_one_metal(TensorView(x_metal), TensorView(y_metal))
            Metal.synchronize()
            ...
            add_one_metal(TensorView(x_strided), TensorView(y_strided))
            Metal.synchronize()
            @test Array(y_strided) ≈ Float32[2, 4, 6, 8]
            ...
            add_one_metal(TensorView(x_mat_metal), TensorView(y_mat_metal))
            Metal.synchronize()
            ...
            add_one_metal(TensorView(x_col), TensorView(y_col))
            Metal.synchronize()
            @test Array(y_col) ≈ Float32[3, 7, 11]
            ...
            add_one_metal(TensorView(x_row), TensorView(y_row))
            Metal.synchronize()
            ...
            add_one_metal(TensorView(x_sub), TensorView(y_sub))
            Metal.synchronize()
