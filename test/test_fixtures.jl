...
@testset "Test Fixtures" begin
    @testset "add_one_cpu - Basic" begin
        ...
        x = Float32[1, 2, 3, 4, 5]
        y = similar(x)
        add_one(from_julia_array(x), from_julia_array(y))
        @test y ≈ Float32[2, 3, 4, 5, 6]

        # Test: [10, 20, 30] + 1 = [11, 21, 31]
        x2 = Float32[10, 20, 30]
        y2 = similar(x2)
        add_one(from_julia_array(x2), from_julia_array(y2))
        @test y2 ≈ Float32[11, 21, 31]

        # Test: zeros + 1 = ones
        x3 = zeros(Float32, 10)
        y3 = similar(x3)
        add_one(from_julia_array(x3), from_julia_array(y3))
    ...
    @testset "add_one_cpu - Strided Arrays" begin
        ...
        add_one(from_julia_array(x_slice), from_julia_array(y_slice))
        ...
        add_one(from_julia_array(x_strided), from_julia_array(y_strided))
        @test y_strided ≈ Float32[2, 4, 6, 8]

        # Test 3: 2D contiguous array
        x_mat = Float32[1 2 3; 4 5 6]  # 2×3
        y_mat = similar(x_mat)
        add_one(from_julia_array(x_mat), from_julia_array(y_mat))
        ...
        y_col = similar(x_col)
        add_one(from_julia_array(x_col), from_julia_array(y_col))
        @test y_col ≈ Float32[3, 7, 11]

        # Test 5: 2D row slice (NON-contiguous, stride > 1)
        x_row = @view mat[2, :]  # [5, 6, 7, 8]
        y_row = similar(x_row)
        add_one(from_julia_array(x_row), from_julia_array(y_row))
        ...
        y_sub = similar(x_sub)
        add_one(from_julia_array(x_sub), from_julia_array(y_sub))
        @test y_sub ≈ Float32[7 8; 11 12]

        # Test 7: 3D array
        x_3d = reshape(Float32[1:24...], 2, 3, 4)  # 2×3×4
        y_3d = similar(x_3d)
        add_one(from_julia_array(x_3d), from_julia_array(y_3d))
        @test y_3d ≈ x_3d .+ 1
        ...
        add_one(from_julia_array(x_3d_slice), from_julia_array(y_3d_slice))
    ...
    @testset "add_one_metal - GPU Arrays (Optional)" begin
        if !METAL_AVAILABLE
            ...
            add_one_metal(from_julia_array(x_metal), from_julia_array(y_metal))
            Metal.synchronize()
            ...
            add_one_metal(from_julia_array(x_strided), from_julia_array(y_strided))
            Metal.synchronize()
            @test Array(y_strided) ≈ Float32[2, 4, 6, 8]
            ...
            add_one_metal(from_julia_array(x_mat_metal), from_julia_array(y_mat_metal))
            Metal.synchronize()
            ...
            add_one_metal(from_julia_array(x_col), from_julia_array(y_col))
            Metal.synchronize()
            @test Array(y_col) ≈ Float32[3, 7, 11]
            ...
            add_one_metal(from_julia_array(x_row), from_julia_array(y_row))
            Metal.synchronize()
            ...
            add_one_metal(from_julia_array(x_sub), from_julia_array(y_sub))
            Metal.synchronize()
